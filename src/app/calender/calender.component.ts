  ...
  private handleRealtimeUpdate(event: BookingEvent): void {
    ...
    switch (event.type) {
      case 'CREATE':
        ...
        this.showNotification(`New booking created by ${event.booking.fullName}`, 'info');
      ...
      case 'DELETE':
        ...
        this.showNotification(`Booking cancelled by ${event.booking.fullName}`, 'info');
        this.updateBusyDates();
        break;
    }
  ...
  book() {
    ...
    this.bookingHistoryService.saveBooking(newBooking).subscribe({
      next: (savedBooking) => {
        ...
        // The real-time update will handle adding to the table
